...
public class ToastsLib extends TwoArgFunction {
    ...
    private Varargs createToast(Varargs varargs) { // title, description, time, icon --> toast
        if (toaster == null)
            return error("Toaster not available");
        ...
        if (!lTitle.isstring())
            return argerror(1, "Expected a string value for argument 'title'");
        if (!lDescription.isstring())
            return argerror(1, "Expected a string value for argument 'description'");
        if (!lTime.isint())
            return argerror(1, "Expected an int value for argument 'time'");
        ChatComponent title;
        String sTitle = lTitle.tojstring();
        if (Util.isResourceName(sTitle))
            title = new ChatComponentText(RockBottomAPI.getGame().getAssetManager().localize(RockBottomAPI.createRes(sTitle)));
        else
            title = new ChatComponentText(sTitle);

        ChatComponent description;
        String sDescription = lDescription.tojstring();
        if (Util.isResourceName(sDescription))
            description = new ChatComponentText(RockBottomAPI.getGame().getAssetManager().localize(RockBottomAPI.createRes(sDescription)));
        else
            description = new ChatComponentText(sDescription);

        int displayTime = lTime.toint();

        if (lIcon.isstring()) {
            String sIcon = lIcon.tojstring();
            if (Util.isResourceName(sIcon))
                return userdataOf(new Toast(RockBottomAPI.createRes(sIcon), title, description, displayTime));
            else
                return argerror(4, "Expected a resource name for argument 'icon'");
        } else if (lIcon.isnil()) {
    ...
    private Varargs displayToast(Varargs varargs) { // toast
        ...
        if (!lToast.isuserdata(Toast.class))
            return argerror(1, "Expected a Toast value for argument 'toast'");
